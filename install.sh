...
#   或仅公网 IP: sudo VPN_SERVER_ID=203.0.113.1 bash install.sh
#   可选: VPN_SERVER_IP=203.0.113.1（域名作 ID 时把公网 IP 写入证书 SAN，便于「用 IP 连接」）
set -eu
...
apt-get update -qq
apt-get install -y pptpd ppp iptables openssl \
  xl2tpd
...
if [ "$FIREWALL_INSTALL" = "yes" ]; then
  ...
  # VPN 相关端口
  iptables -A INPUT -p gre -j ACCEPT  # PPTP
...
#!/bin/bash
# VPN 服务守护脚本
# 检查 PPTP 和 L2TP 服务状态，如果服务未运行则自动启动
...
放行端口:
  - VPN 相关: PPTP (TCP 1723 + GRE)、L2TP (UDP 1701)
